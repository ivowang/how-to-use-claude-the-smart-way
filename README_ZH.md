...
如果您是人工智能行业的研究人员或博士，声明本地 GPU 环境、网络代理和 Python 环境等事项也很重要。这些内容有助于 Claude 在您的本地机器上运行实际测试和实验。下面是一份起始模板，你可以修改后粘贴到自己的 repo 里：
...
剩下三个项目留作手动步骤，是因为它们的安装命令依赖的细节(包名、MCP 传输、凭证)变动得比较频繁，硬编码在这里不安全，对应的 repo 里有最新的指引。
...
**让 Claude 自己维护一份开发日志。** 对任何跨越很多会话的长程项目，让 Claude 在 `docs/dev/` 目录下自己记笔记：每遇到一个值得记录的问题，就写一份短小的文件，记下症状、试过什么、最后是什么 work 了、以及下次值得记住的那一句教训。下面这个 prompt 可以让 claude 把这个习惯建立起来：
...
每一份笔记都是廉价的，几乎不需要维护，但它们会慢慢复利。跨越许多次会话之后，Claude 会开始在重复你的调试之前先查自己过去的笔记、在多个 bug 之间发现模式、甚至在你快要踩进一个已经被记录过的坑时主动把你拦下来。在一个很小但很真实的意义上，这就是 agent 在 memory 层面的 **self-evolving**：每一次会话结束的时候，这个 agent 都在"你这个具体项目上"比会话开始时多了一点经验。你不需要 fine-tuning，也不需要设计复杂的长程记忆架构，一个 markdown 文件夹加上让 Claude 先去读它的 prompt，就已经拿到了大部分的价值。
